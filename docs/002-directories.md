...
Did you catch the new thing I threw in there? Yep, it's the "`..`" shortcut! The ".." shortcut refers to the **parent directory** of the current directory. So typing "`cd ..`" will get you to a **different place** depending on where you are in the directory structure when you type it.
...
## Unix Philosophy -- The Value of not Being Verbose

This is a good opportunity to talk about a part of the Unix philosophy -- the value of being terse.
...
This is because Unix tools are designed to be chained together -- to have the output of one command fed into the input of another command. For instance, (we'll cover this in detail later) you can pipe the output of `ls` to an application that sorts that output, and then to another application that counts the number of lines and outputs that. This is an incredibly powerful set of tools that you can use to do amazing things -- but it can seem deliberately obtuse and annoying at first.
